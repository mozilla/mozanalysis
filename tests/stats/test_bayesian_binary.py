...
def test_aggregate_col():
    ...
    res1 = masbb.aggregate_col(df, 'val1')
    ...
    res2 = masbb.aggregate_col(df, 'val2')
...
def test_summarize_one_branch_vs_samples():
    ...
    ppf = masbb.summarize_one_branch_from_agg(s, quantiles=[0.5, 0.41])

    samples = masbb.get_samples(
        s.to_frame().T, 'num_enrollments', 'num_conversions', 100000
    )

    res = masss.summarize_one_branch_samples(samples.iloc[:, 0], quantiles=[0.5, 0.41])
...
def test_compare_branches():
    ...
    res = masbb.compare_branches(df, 'val2', ref_branch_label='ctl')
