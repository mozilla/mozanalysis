...
from mozanalysis.experiment import AnalysisBasis, AnalysisWindow, TimeLimits
...
def test_multiple_datasource():
    ...
    test_sds = SegmentDataSource('test_ds',
                                'test_table',
                                )
    test_seg = Segment('test_seg',
                        test_sds,
                        "TEST AGG SELECT STATEMENT",
                        '','')
    target_sql = test_target.build_targets_query(
        time_limits=tl, 
        target_list=[msd.new_unique_profiles, test_seg])
...
def test_query_not_detectably_malformed():
    ...
    target_sql = test_target.build_targets_query(
        time_limits=tl, 
        target_list=[mad.new_unique_profiles])
...
def test_megaquery_not_detectably_malformed():
    ...
    metrics_sql = test_target.build_metrics_query(
        time_limits=tl,
        metric_list=[m for m in mad.__dict__.values() if isinstance(m, mad.Metric) and 'experiment_slug' not in m.select_expr],
        targets_table="targets")
...
def test_custom_query_override_target():
    ...
    target_sql = test_target.build_targets_query(
        time_limits=tl, 
        target_list=[test_seg],
        custom_targets_query=custom_query)

    assert ("custom_query_target_table" in target_sql and 
