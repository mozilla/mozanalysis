...
def test_resample_and_agg_once_bcast(spark_context):
    b_data = spark_context.broadcast(np.array([3.0, 3.0, 3.0]))
    assert mafsb._resample_and_agg_once_bcast(b_data, np.mean, 42) == 3.0


def test_get_bootstrap_samples(spark_context_or_none):
    res = mafsb.get_bootstrap_samples(
        np.array([3.0, 3.0, 3.0]), num_samples=2, sc=spark_context_or_none
    )
    assert res.shape == (2,)
